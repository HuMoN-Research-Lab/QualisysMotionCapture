...
% VideoFrameRate     Frames/sec                                             [COMMENTED OUT]
% AnalogSignals      Analog signals [Nsignals x NanalogSamples]             [COMMENTED OUT]
% AnalogFrameRate    Samples/sec                                            [COMMENTED OUT]
% Event              Event(Nevents).time ..value  ..name                    [COMMENTED OUT]
% ParameterGroup     ParameterGroup(Ngroups).Parameters(Nparameters).data ..etc.
% CameraInfo         MarkerRelated CameraInfo [Nmarkers x NvideoFrames]     [COMMENTED OUT]
% ResidualError      MarkerRelated ErrorInfo  [Nmarkers x NvideoFrames]     [COMMENTED OUT]
...
ind = findstr(c3dFileName,'\');
if ind>0 
    c3dFileName = c3dFileName(ind(length(ind))+1:length(c3dFileName));
else
    c3dFileName = c3dFileName;
end
...
if key~=80
    h=errordlg(['File: ',c3dFileName,' does not comply to the C3D format'],'application error');
    uiwait(h)
    fclose(fid);
    return
end
...
if proctype==2
    fclose(fid);
    fid=fopen(c3dFileName,'r','d'); % DEC VAX D floating point and VAX ordering
end
...
h = waitbar(0,[c3dFileName,' is loading...']);
...
if Scale < 0
    for i=1:NvideoFrames
        for j=1:Nmarkers
%             data.Markers(1:3,i,j)=fread('float32',fid,3)'; 
            Markers(i,j,1:3)=fread(fid,3,'float32')'; 
%             a=fix(fread('float32',fid,1));  
...
%         end
    ...
    for i=1:NvideoFrames
        for j=1:Nmarkers
%             data.Markers(1:3,i,j)=fread('int16',fid,3)'.*Scale;
            data.Markers(i,j,1:3)=fread(fid,3,'int16')'.*Scale;
