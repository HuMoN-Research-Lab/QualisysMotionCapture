function hhh=vline(x,in1,in2, yloc)
...
if length(x)>1  % vector input
    for I=1:length(x)
        ...
        h(I)=vline(x(I),linetype,label, yloc);
    end
else
    switch nargin
    case 1
    ...
    y=get(gca,'ylim');
%     h=plot([x x],y,linetype, 'LineStyle', ':');
    ...
    if length(label)
        ...
        xunit=(x-xx(1))/xrange;
        if xunit<0.8
            text(x-0.01*xrange,y(1)+yloc*(y(2)-y(1)),label,'color',get(h,'color'), 'FontSize', 8)
        else
            text(x-.05*xrange,y(1)+yloc*(y(2)-y(1)),label,'color',get(h,'color'), 'FontSize', 8)
        end
    end     
