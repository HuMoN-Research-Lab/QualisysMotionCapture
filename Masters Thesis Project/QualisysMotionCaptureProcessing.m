...
%% Experiment Info 
cond1 = 1:47; % Trials 1-47 were free walking (FW) trials
...
for trial_num = trial_cond
    ...
    %% Indexes seg_center data using trial start and end frames
    [seg_center_cal] = index_seg_center(seg_center,trial_start_end,marker_mar_dim_frame,marker_labels);
    
    %% Indexes force data using trial start and end frames
...
% % Barplot of change in acceleration along kinematic chain
% barplot_trial_steps_acc(head_trials_steps_acc,chest_trials_steps_acc,hip_trials_steps_acc,LThigh_trials_steps_acc,RThigh_trials_steps_acc,LLeg_trials_steps_acc,RLeg_trials_steps_acc,LFoot_trials_steps_acc,RFoot_trials_steps_acc,trial_num);

% % Barplot of change in jerk along kinematic chain
...
%% Experiment Statistical Evaluation
...
% % Statistical analysis of overall experiment
...
%% Work To Be Done
% % Create a cell array of the normalized step data
...
%     plotMoCapTracer(seg_center,fr)
    ...
    %% Plotting Lower Extremities Sections
    %Plot of right side of human skeleton
    for plot_lower_ext_section = true       
        LHipCenter =        seg_center_cal.LHipCenter;%segCenterVariable.LHipCenter;
        RHipCenter =        seg_center_cal.RHipCenter;
        LThigh =            seg_center_cal.LThigh;
        RThigh =            seg_center_cal.RThigh;
        LKnee =             seg_center_cal.LKnee; %inactive
        RKnee =             seg_center_cal.RKnee;
        LAnkle =            seg_center_cal.LAnkle;
        RAnkle =            seg_center_cal.RAnkle;
        LToeTip =           seg_center_cal.LToeTip;
        RToeTip =           seg_center_cal.RToeTip;
    end    
        
    plot3([RHipCenter(1,fr);RKnee(1,fr);RAnkle(1,fr);RToeTip(1,fr)],...
        [RHipCenter(2,fr);RKnee(2,fr);RAnkle(2,fr);RToeTip(2,fr)],...
        [RHipCenter(3,fr);RKnee(3,fr);RAnkle(3,fr);RToeTip(3,fr)],'-r','LineWidth',2)
    
    hold on
    %Plot of left side of human skeleton
    plot3([LHipCenter(1,fr);LKnee(1,fr);LAnkle(1,fr);LToeTip(1,fr)],...
        [LHipCenter(2,fr);LKnee(2,fr);LAnkle(2,fr);LToeTip(2,fr)],...
        [LHipCenter(3,fr);LKnee(3,fr);LAnkle(3,fr);LToeTip(3,fr)],'-b','LineWidth',2)
